...
public class Six_BinarySearchTree<Key extends Comparable<Key>, Value> {
	private Node root;
    ...
    public Iterable<Key> keys(Key lo, Key hi) {
        Six_Queue<Key> queue = new Six_Queue<Key>();
        keys(root, queue, lo, hi);
        return queue;
    } 

    private void keys(Node x, Six_Queue<Key> queue, Key lo, Key hi) { 
        if (x == null)
        	return; 
