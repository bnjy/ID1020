...
public class Three_BinarySearchTree<Key extends Comparable<Key>, Value> {
	private Node root;
    ...
    public Iterable<Key> keys(Key lo, Key hi) {
        Three_Queue<Key> queue = new Three_Queue<Key>();
        keys(root, queue, lo, hi);
        return queue;
    } 

    private void keys(Node x, Three_Queue<Key> queue, Key lo, Key hi) {
        if (x == null)
        	return; 
